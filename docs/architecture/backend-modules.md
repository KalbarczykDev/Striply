...
- Modules may use another module only through its `api`.
- Domain code may depend on Java and stable `shared` primitives.
- Infrastructure may depend inward on application and domain.
...
- `shared` contains no business workflows.
- Global `infrastructure` contains no domain behavior.
...
The table defines ownership, not a commitment to create every listed class during VS-001. A public responsibility becomes an implemented contract only when a ticket defines its inputs, outputs, failure behavior and authorization boundary.
